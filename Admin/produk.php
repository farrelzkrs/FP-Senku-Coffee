...
<head>
    ...
    <style>
        ...
        .sidebar .navbar-logo {
            height: 65px;
            /* Tinggi logo sesuai dengan navbar */
            width: auto;
        ...
        .sidebar .nav-link:hover {
            background: beige;
            /* Latar belakang saat aktif atau hover */
            color: #006400 !important;
            /* Warna teks saat aktif atau hover */
        }
        ...
        .dropdown-menu-profil .dropdown-item {
            ...
            font-weight: bold;
            /* Tebalkan teks item dropdown */
            font-size: 15px;
            /* Ukuran font item dropdown */
        }
    ...
    <div class="main-content">
        ...
        <div class="konten">
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($row['gambar_url']): ?>
                                    <img src="../uploads/<?= htmlspecialchars($row['gambar_url']) ?>" alt=""
                            ...
                            <td>
                                ...
                                <div class="modal fade" id="hapusModal<?= $row['id_produk'] ?>" tabindex="-1"
                                    aria-labelledby="hapusModalLabel<?= $row['id_produk'] ?>" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <form method="post" action="hapus_produk.php" class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="hapusModalLabel<?= $row['id_produk'] ?>">
                                                    Konfirmasi Hapus Produk</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                    aria-label="Tutup"></button>
                                            </div>
                                            <div class="modal-body">
                                                <p>Masukkan password admin untuk menghapus produk
                                                    <b><?= htmlspecialchars($row['nama_produk']) ?></b>?</p>
            ...
            <table class="table table-bordered table-striped align-middle">
                ...
                <tbody>
                    <?php $no = 1 + $offset_jeniskopi;
                    while ($row = $jeniskopi->fetch_assoc()): ?>
                        <tr>
                            <td><?= $no++ ?></td>
                            <td><?= htmlspecialchars($row['nama_jenis']) ?></td>
                            <td>
                                <?php if ($row['gambar_kopi']): ?>
                                    <img src="../uploads/<?= htmlspecialchars($row['gambar_kopi']) ?>" alt=""
                                        style="width:60px; height:60px; object-fit:cover;">
                                <?php endif; ?>
                            </td>
                            <td><?= htmlspecialchars($row['deskripsi']) ?></td>
                            <td>
                                ...
                                <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal"
                                    data-bs-target="#hapusJenisModal<?= $row['id_kopi'] ?>">
                                    <i class="bi bi-trash"></i> Hapus
                                </button>

                                <!-- Modal Konfirmasi Hapus Jenis Kopi -->
                                <div class="modal fade" id="hapusJenisModal<?= $row['id_kopi'] ?>" tabindex="-1"
                                    aria-labelledby="hapusJenisModalLabel<?= $row['id_kopi'] ?>" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <form method="post" action="hapus_produk.php" class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="hapusJenisModalLabel<?= $row['id_kopi'] ?>">
                                                    Konfirmasi Hapus Jenis Kopi</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                    aria-label="Tutup"></button>
                                            </div>
                                            <div class="modal-body">
                                                <p>Masukkan password admin untuk menghapus jenis kopi
                                                    <b><?= htmlspecialchars($row['nama_jenis']) ?></b>?</p>
                    ...
                    <ul class="pagination justify-content-center">
                        <?php for ($i = 1; $i <= $total_page_produk; $i++): ?>
                            <li class="page-item <?= $i == $page_produk ? 'active' : '' ?>">
                                <a class="page-link"
                                    href="?page_produk=<?= $i ?>&page_jeniskopi=<?= $page_jeniskopi ?>"><?= $i ?></a>
                            </li>
                        <?php endfor; ?>
                    </ul>
                </nav>
                    ...
                    <ul class="pagination justify-content-center">
                        <?php for ($i = 1; $i <= $total_page_jeniskopi; $i++): ?>
                            <li class="page-item <?= $i == $page_jeniskopi ? 'active' : '' ?>">
                                <a class="page-link"
                                    href="?page_produk=<?= $page_produk ?>&page_jeniskopi=<?= $i ?>"><?= $i ?></a>
                            </li>
                        <?php endfor; ?>
                    </ul>
                </nav>
