...
// Dapatkan nama file halaman aktif
$current = basename($_SERVER['PHP_SELF']);

// Cek login user (misal session 'user' di-set saat login)
$is_logged_in = isset($_SESSION['user_id']);
$foto_profil = '../Resource/fotoprofil2.jpg'; // Default foto profil
$nama_user = isset($_SESSION['username']) ? $_SESSION['username'] : 'Profil';

if ($is_logged_in) {
    // Jika ingin ambil nama/foto dari database, bisa query di sini
    // Contoh:
    $id = $_SESSION['user_id'];
    $q = $conn->query("SELECT username, photo_profil FROM users WHERE id_users=$id");
    if ($row = $q->fetch_assoc()) {
        $nama_user = $row['username'];
        $foto_profil = $row['foto'] ? '../uploads/'.$row['foto'] : $foto_profil;
    }
}
?>
<div class="header">
                ...
                <ul class="navbar-nav ms-2">
                    <li class="nav-profil-item dropdown dropdown-profil">
                        <a class="nav-link dropdown-toggle d-flex align-items-center" href="#" id="navbarProfileDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                            <img src="<?= $foto_profil ?>" alt="Profil" class="rounded-circle" width="47" height="47" style="object-fit: cover; margin-right: 10px;">
